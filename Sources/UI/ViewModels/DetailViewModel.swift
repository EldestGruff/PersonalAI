...
final class DetailViewModel {
    ...
    func saveChanges() {
        ...
        _Concurrency.Task {
            do {
                ...
                AppLogger.error("DetailViewModel: save failed", category: .general)
                AppLogger.debug("DetailViewModel: error type logged", category: .general)
                if let validationError = error as? ValidationError {
                    AppLogger.debug("DetailViewModel: validation error details", category: .general)
                }
    ...
    func requestAction() {
        ...
        if autoCreate {
            ...
            confirmationTitle = EventHelpers.extractCleanTitle(from: thought.content, parsedDateTime: classification.parsedDateTime)
            let (startDate, _) = EventHelpers.calculateEventTimes(from: classification.parsedDateTime)
    ...
    func createReminderOrEvent(titleOverride: String? = nil, startDateOverride: Date? = nil, durationOverride: Int? = nil) {
        ...
        _Concurrency.Task {
            do {
                ...
                let cleanTitle = titleOverride ?? EventHelpers.extractCleanTitle(
                ...
                let dueDate: Date? = startDateOverride ?? {
                    let (date, _) = EventHelpers.calculateEventTimes(from: classification.parsedDateTime)
                ...
                if classification.type == .reminder {
                    ...
                    if let overrideStart = startDateOverride {
                        ...
                        (eventStart, eventEnd) = EventHelpers.calculateEventTimes(from: classification.parsedDateTime)
                    }
                ...
                AppLogger.error("DetailViewModel: error creating reminder/event", category: .general)
                AppLogger.debug("DetailViewModel: error type logged", category: .general)
                self.error = AppError.from(error)
                AppLogger.debug("DetailViewModel: error converted to AppError", category: .general)
            }
